...
sub proc_line
{
  ...
  my($From);
  my($To);
  if        ($param{line_rec}{line}[ $main::name2pos{ 'Type' } ] eq 4) # outgoing
    {
  ...
  elsif(    ($param{line_rec}{line}[ $main::name2pos{ 'Type' } ] eq 1) # incoming …
	 || ($param{line_rec}{line}[ $main::name2pos{ 'Type' } ] eq 2) # incoming …
       )
    {
